import emailjs from "@emailjs/browser" // Declare the emailjs variable
...
export class FormModule {
  ...
  async handleSubmit(e) {
    ...
    if (this.submitAttempts >= this.maxAttempts) {
      this.showNotification("Demasiados intentos. Contacta directamente a diego.barrera@sento.tech", "error", 8000)
      return
    }
    ...
    try {
      ...
      const templateParams = {
        ...
        company: formData.get("company") || "No especificado",
      ...
      const errorMessage =
        this.submitAttempts >= this.maxAttempts
          ? "Error persistente. Contacta directamente a diego.barrera@sento.tech"
          : `Error al enviar (intento ${this.submitAttempts}/${this.maxAttempts}). Intenta nuevamente.`
  ...
  showNotification(message, type = "info", duration = 5000) {
    ...
    notification.style.cssText = `
      ...
      color:white;padding:1rem 1.5rem;border-radius:0.5rem;box-shadow:0 20px 25px -5px rgba(0,0,0,0.1);
      z-index:10000;max-width:350px;transform:translateX(100%);
      transition:transform 0.3s cubic-bezier(0.4,0,0.2,1);font-size:0.875rem;
    `
